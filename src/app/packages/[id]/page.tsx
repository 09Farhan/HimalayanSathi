        ...
        <div className="absolute inset-0 z-10 flex flex-col justify-end pb-12 px-4 md:px-8 max-w-7xl mx-auto w-full">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            <span className="bg-white/20 backdrop-blur-md px-3 py-1 text-white text-sm rounded-full">
              {(Array.isArray(pkg.type) ? pkg.type : [pkg.type]).map((t: string) => (t || '').charAt(0).toUpperCase() + (t || '').slice(1)).join(' & ')} Tour
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 md:gap-6 divide-x-0 md:divide-x divide-surface-muted">
            ...
            <div className="flex items-center gap-3 md:gap-4 md:pl-6">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="font-bold text-sm md:text-base text-primary-dark capitalize truncate">{(Array.isArray(pkg.type) ? pkg.type : [pkg.type]).join(', ')}</p>
              </div>
            </div>
          </div>
        </div>
